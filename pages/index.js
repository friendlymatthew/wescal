...
export default function Home() {
  ...
  const getSpring2020 = () => {
    axios
      .get("https://safe-lowlands-86945.herokuapp.com/api/v1/archive/spring2022")
      .then((response) => {
        setEvents(response.data);
      })
      .catch((error) => console.log(error));
        ...
        <section className="space-x-6 rounded-3xl container border-2 bg-indigo-500 p-2 max-w-2xl">
          <div class="dropdown dropdown-hover">
            <div
              onClick={getAll}
              tabindex="0"
              class="m-1 text-black bg-indigo-500 hover:text-white font-bold py-2 px-4 rounded transform hover:scale-125 transition ease-out duration-300"
            >
              Archive
            </div>
            <ul
